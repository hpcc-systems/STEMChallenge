...
EXPORT FN_GetPrice(Zip, Assess_val, Year_acq, 
		...
		mymodel := DATASET('~class::bmf::whshp::mymodelR',ML_Core.Types.Layout_Model2,FLAT,PRELOAD);

		myLearner := LT.RegressionForest(10,,10,[1]);

		myPredictDeps := MyLearner.Predict(myModel, myIndepData);

     // Round_Value := TRUNCATE(myPredictDeps.value);
	
		RETURN OUTPUT(myPredictDeps,{ROUND(value)});
